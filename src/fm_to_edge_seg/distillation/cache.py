...
def create_reference_teacher_cache(
    ...
    for record in records:
        ...
        logits = np.log(probability / (1.0 - probability)).astype(np.float16)
        confidence = valid.astype(np.float16)
        filename = hashlib.sha256(record.sample_id.encode("utf-8")).hexdigest() + ".npz"
        destination = samples_directory / filename
        if destination.exists() and not overwrite:
            raise FileExistsError(f"Refusing to replace teacher sample: {destination}")
        np.savez_compressed(destination, logits=logits, confidence=confidence)
        rows.append({"sample_id": record.sample_id, "cache_path": f"samples/{filename}"})
